...
Inside our Gameplay Loop, we'll refactor the bottom of the screen's collision detection to now be the "Floor" of our Infinite Runner. Meanwhile, we'll keep the left and right "Walls" as boundaries for our player character to move within horizontally, as well as the same code for our left and right controls. Up and down are going to be commented out for now, since the character will be grounded while running. Our "Jump" Button can increase the Y-Position (or technically decrease it, since 0 is at the top), like pressing "Up" did before, but in a larger increment initially. 
