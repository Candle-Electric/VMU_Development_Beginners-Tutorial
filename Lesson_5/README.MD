...
Inside our Gameplay Loop, we'll refactor the bottom of the screen's collision detection to now be the "Floor" of our Infinite Runner. Meanwhile, we'll keep the left and right "Walls" as boundaries for our player character to move within horizontally, as well as the same code for our left and right controls. Up and down are going to be commented out for now, since the character will be grounded while running. A can be our "Jump" Button to increase the Y-Position (or technically decrease it, since 0 is at the top), like pressing "Up" did before, but in a larger increment initially. 
...
Then, "Gravity" can pull the Sprite back down to the Floor, by subtracting an "Accelerated" Value from the Y-Position. To start, we'' instantiate a new variable. Every Frame, we'll decrement this, and when it rolls over past zero, the overload will take care of it for us + have the Player change from rising to falling.
